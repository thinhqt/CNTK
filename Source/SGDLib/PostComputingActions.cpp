    ...
    const wstring newModelPath, const size_t mbSize, const int iters)
{
    ...
    for (auto& node : bnNodes)
    {
        ...
        if (useDistributedMBReading)
            dataReader->StartDistributedMinibatchLoop(mbSize, epoch++, m_mpi->CurrentNodeRank(), m_mpi->NumNodesInUse());
        else
            dataReader->StartMinibatchLoop(mbSize, epoch++);
        ...
        for (int iter = 0; iter < iters; iter++)
        {
            ...
            if (!wasDataRead)
            {
                ...
                wasDataRead = DataReaderHelpers::GetMinibatchIntoNetwork<ElemType>(*dataReader, m_net,
                    nullptr, useDistributedMBReading, useParallelTrain, inputMatrices, actualMBSize, m_mpi);
            }
